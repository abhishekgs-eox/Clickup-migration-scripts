...
def update_description_with_attachments_clickup_native(clickup_task_id, original_description, uploaded_files, task_id):
    """Update task description with clean content and INLINE attachment previews"""
    try:
        print(f"   🔄 Creating enhanced description with INLINE attachment previews...")
        ...
        # Step 3: Build final description with INLINE image previews
        final_description = ""
        ...
        # Step 4: Add inline image previews for image files
        if uploaded_files:
            ...
            for file_info in uploaded_files:
                if isinstance(file_info, dict):
                    ...
                    attachment_url = file_info.get('url', '')
            ...
            if images:
                ...
                for img_info in images:
                    ...
                    # Clean filename - remove ID prefix
                    clean_name = filename
                    if '_' in filename:
                    ...
                    # Embed image inline using ClickUp's image syntax
                    if url:
                        final_description += f"![{clean_name}]({url})\n"
                        final_description += f"*{clean_name}*\n\n"
                    else:
                        final_description += f"📷 {clean_name}\n\n"
            
            # Add other file attachments as links
            if other_files:
        ...
        preview = final_description[:100] + ('...' if len(final_description) > 100 else '')
        print(f"   📝 Final description with inline images preview: '{preview}'")
        print(f"   📝 Total description length: {len(final_description)} characters")
        
        # Step 5: Update the task description
        ...
        if response.status_code == 200:
            print(f"   ✅ Task description updated with INLINE image previews!")
            return True, None
        else:
...
def upload_all_attachments_advanced(clickup_task_id, attachments, original_description="", task_id=""):
    """Advanced attachment upload with enhanced description handling"""
    uploaded_count = 0
    attachment_errors = []
    uploaded_files = []
    
    print(f"📎 Processing attachments and description for task...")
    
    # Upload attachments if any
    if attachments:
        print(f"📎 Starting upload of {len(attachments)} attachment(s)...")
    ...
    # Update description (whether we have attachments or not)
    if original_description or uploaded_files:
        print(f"   🖼️ Updating task description...")
...
def main():
    ...
    print("🖼️ Enhanced descriptions with INLINE attachment previews enabled")
    ...
    for index, row in df_sorted.iterrows():
        ...
        if task_response:
            ...
            # Upload attachments and update description with enhanced format
            uploaded_count = 0
            attachment_errors = []
            ...
            if attachments:
                ...
                df_sorted.at[index, 'description_updated'] = 'Yes with Inline Images'
            else:
    ...
    inline_images_added = len(df_sorted[df_sorted['description_updated'] == 'Yes with Inline Images'])
    ...
    print(f"🖼️ Tasks with INLINE image previews: {inline_images_added}")
    ...
    if successful_tasks > 0:
        ...
        if inline_images_added > 0:
            print(f"🖼️ {inline_images_added} tasks now have INLINE image previews in descriptions!")
...
if __name__ == "__main__":
    ...
    print("   • 🖼️ INLINE image previews in task descriptions")
    ...
    print("   • Clean task descriptions with INLINE image previews")
